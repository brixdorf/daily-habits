...
const COLORS = ['#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899', '#f97316', '#14b8a6', '#6366f1'];

function nextColor() {
  const used = db.prepare('SELECT color FROM habits').all().map(r => r.color);
...
router.post('/', (req, res) => {
  ...
  const info = db.prepare('INSERT INTO habits (name, goal, color) VALUES (?, ?, ?)').run(
    name.trim(), parseInt(goal, 10), color
  );
...
router.post('/:id/toggle', (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM habit_checks WHERE habit_id = ? AND date = ?').get(habitId, date);
  if (existing) {
    db.prepare('DELETE FROM habit_checks WHERE id = ?').run(existing.id);
    res.json({ checked: false });
  } else {
    db.prepare('INSERT INTO habit_checks (habit_id, date) VALUES (?, ?)').run(habitId, date);
